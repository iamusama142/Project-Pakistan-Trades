...
<body>
    ...
    <center>
        ...
        <p style="font-size: 17px;">Purchaser Name:
            ...
            @endforeach
            <?php $posts = App\Models\pakTrader::latest()->first('sellername');
            ?>

            <span style="color: red; font-size:20px;">{{ $posts->sellernamephp}}</span>
        </p>
    ...
    <table style="width:100%">
        <tr>
            ...
            <td class="c" colspan="1">Sellername Name:</td>
        ...
        @foreach ($PDFReport as $PDFReports)
            ...
            <tr>
                ...
                <td class="c" colspan="1">{{ $PDFReports->s_firstwt }}</td>
                <td class="c" colspan="1">{{ $PDFReports->s_bourirate }}</td>
                <td class="c" colspan="1">{{ $PDFReports->s_thelarate }}</td>
                <td class="c" colspan="1">{{ $PDFReports->s_bwt }}</td>
                <td class="c" colspan="1">{{ $PDFReports->s_ewt }}</td>
                <td class="c" colspan="1">{{ $PDFReports->s_rate }}</td>
                <td class="c" colspan="1">{{ $PDFReports->s_vrent }}</td>
                <td class="c" colspan="1">{{ $PDFReports->s_netamount }}</td>
            </tr>
            @endforeach
            @endfor
